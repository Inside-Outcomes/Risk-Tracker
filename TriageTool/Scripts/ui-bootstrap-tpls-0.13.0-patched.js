...
.service('dateParser', ['$locale', 'orderByFilter', function($locale, orderByFilter) {
  ...
  this.parse = function(input, format, baseDate) {
    ...
    var monthformats = ['MMMM', 'MMM', 'MM', 'M'];
    var dayformats = ['dd', 'd'];
    var seps = ['/', '-', ' '];
    for (var i = 0; i != yearformats.length; ++i)
        for (var j = 0; j != monthformats.length; ++j)
            for (var k = 0; k != dayformats.length; ++k)
                for (var l = 0; l != seps.length; ++l)
                    formats.push(dayformats[k]+seps[l]+monthformats[j]+seps[l]+yearformats[i]);
    ...
    for (var f = 0; f != formats.length; ++f) {
        ...
        if (results && results.length) {
            ...
            if (isValid(fields.year, fields.month, fields.date)) {
                dt = new Date(Date.UTC(fields.year, fields.month, fields.date));
