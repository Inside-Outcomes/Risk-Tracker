...
namespace RiskTracker.Reports {
  public class OpenDataReport {
    public static ReportData<Row> build(Guid orgId, Guid projId, DateTime? startDate, DateTime? endDate) {
      ...
      foreach (var client in clients) {
        ...
        foreach (var rad in client.RiskAssessments) {
          ...
          row.bump();
          var assessed = rad.Risks().Select(riskId => riskMap.Risks().Where(r => r.Id == riskId).Single().Title).ToList();
          row.assessed(assessed);
          var resolved = rad.ResolvedRisks().Select(riskId => riskMap.Risks().Where(r => r.Id == riskId).Single().Title).ToList();
    ...
    public static string buildCSV(Guid orgId, Guid projId, DateTime? startDate, DateTime? endDate) {
